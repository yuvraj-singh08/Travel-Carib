...
export const sendEmail = async (bookingData: any) => {
  ...
  let processedBookingData = {
    ...bookingData,
    passenger: bookingData.passenger.map((passengerStr: string) => {
      try {
        return JSON.parse(passengerStr);
      } catch (error) {
  ...
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "mockmail36@gmail.com",
      pass: "ojqvojqzpvowsxea",
    },
  });
...
// export const sendEmail = async (bookingData:any) => {
//   console.log("booking data in emailService: ", bookingData)
...
//       const recipientEmail = bookingData.contactDetail?.email || bookingData.passenger[0]?.email;
//       if (!recipientEmail) {
//         throw new Error("Recipient email is missing in booking data");
//       }
